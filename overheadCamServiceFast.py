...
def detectMarkers(cam):
    cam._cam.startCapture()
    while True:
        try:
            image = cam._cam.retrieveBuffer()
        ...
        corners, ids, rejectedImgPoints = aruco.detectMarkers(gray, cam._aruco_dict, parameters=cam._arucoParams)
        ...
        if ids is None:
            ...
            for m in ids:
                IDS.append(m[0])
            for m in corners:
                pc_temp, Rc_temp = cam.getObjectPose(m, cam._markerSize, cam._camMatrix, cam._distCoeff) #88.5
...
def main():
    ...
    updateMarkers = threading.Thread(target=detectMarkers,args = (overheadCam,))
